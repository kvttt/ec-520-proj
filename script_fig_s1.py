...
df = pd.DataFrame(columns=[
    "patch_size", "patch_distance", "MSE", "PSNR", "SSIM", "Method"
])
...
for patch_size in [3, 5, 7, 9, 11]:
    for patch_distance in [5, 7, 9, 11, 13]:
        u_nlm, mse_nlm, psnr_nlm, ssim_nlm = get_result_gray(nlm, u_noisy, u_clean, h=0.1, patch_size=patch_size, patch_distance=patch_distance)
...
for patch_size in [3, 5, 7, 9, 11]:
    for patch_distance in [5, 7, 9, 11, 13]:
        u_nlm_rgb, mse_nlm_rgb, psnr_nlm_rgb, ssim_nlm_rgb = get_result_rgb(nlm, u_noisy, u_clean, h=0.18, patch_size=patch_size, patch_distance=patch_distance)
...
for patch_size in [3, 5, 7, 9, 11]:
    for patch_distance in [5, 7, 9, 11, 13]:
        u_nlm_lab, mse_nlm_lab, psnr_nlm_lab, ssim_nlm_lab = get_result_lab(nlm, u_noisy, u_clean, h=18, patch_size=patch_size, patch_distance=patch_distance)
