"""
File Name: script_fig_11.py
Author: Jiatong
Function: Generates the color salt-and-pepper qualitative comparison figure and subplot NPZ files.
Reference: None.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Plot the color salt-and-pepper appendix figure.")
    parser.add_argument(
        "--checkpoint",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=project_root / "figures" / "fig11" / "fig11.png",
...
def main() -> None:
    ...
    u_nlm, mse_nlm, psnr_nlm, ssim_nlm, perceptual_nlm = get_result_rgb(nlm, u_noisy, u_clean, h=nlm_h)
    u_bf, mse_bf, psnr_bf, ssim_bf, perceptual_bf = get_result_rgb(
        bf,
        u_noisy,
        u_clean,
    ...
    u_dncnn, mse_dncnn, psnr_dncnn, ssim_dncnn, perceptual_dncnn = get_result_rgb(
        dncnn_denoise,
        u_noisy,
        u_clean,
    ...
    print(f"Saved figure to: {args.output} (dpi={result['dpi']}, bytes={result['bytes']})")
